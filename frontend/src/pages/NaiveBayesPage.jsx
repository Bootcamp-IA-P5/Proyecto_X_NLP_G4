      ...
      <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Modelo probabilístico muy rápido, ideal como baseline para texto.
          </p>
        </div>
      </header>
      ...
      <section className="flex-1 grid grid-cols-1 lg:grid-cols-[2.5fr_1fr] gap-6">
        <div className="bg-slate-900/80 border border-slate-800 rounded-3xl shadow-xl p-4 md:p-6 flex flex-col">
          <p className="text-xs text-slate-400 mb-3">
            Escribe un comentario y Naive Bayes te dirá si es tóxico (1) o no
            tóxico (0).
          </p>
          <ModelChat modelKey="naive_bayes" />
        </div>
        ...
        <aside className="bg-slate-900/80 border border-slate-800 rounded-3xl shadow-xl p-5 text-xs text-slate-300 space-y-2">
          ...
          <p>
            Naive Bayes asume independencia entre palabras pero en la práctica
            funciona sorprendentemente bien en tareas de clasificación de texto.
          </p>
        </aside>
      </section>
    </div>
